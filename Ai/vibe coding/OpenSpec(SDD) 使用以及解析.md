...
> **解决的问题**
> 
> Ai编码最大的问题是需求的关键词之后在全局模糊搜索（猜+模糊搜），这样的不仅效率很低，而且很多时候会加载无关信息导致出现严重的幻觉。
> OpenSpec 解决的问题是从需求到代码的规范，我们可以基于 OpenSpec 写一个十分具体的ai行动流程，极大限度规定ai活动的范围，使得ai可以编写相对高质量的代码。

>[!question] 工具局限性
>OpenSpec 确实可以解决上述问题，但是他对项目的架构分析等等文档都需要用户自己维护，关于这个方面应该引入ai互动模式和用户不断交互完善该文档。
> **存在的问题**
> 
> OpenSpec 的文档规范在ai完成任务后只会保留相关模块(能力)的业务逻辑，不会保留任何相关的代码，导致下次change来临的时候，又去采用 猜 + 搜 的方式去寻找相关实现（**可进行改进**）
